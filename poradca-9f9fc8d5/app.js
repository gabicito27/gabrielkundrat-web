...
const hvPoradca = (id, meno, monogram, rola = "finančný sprostredkovateľ") => ({
  id, meno, rola, monogram,
  logo: HV_LOGO, logoPomer: HV_POMER, kontakt: HV_KONTAKT, farby: HV_FARBY
});
...
const PORADCOVIA = [
  ...
  hvPoradca("hv", "Hypotéka Východ",         "HV", "finančné sprostredkovanie"),
  hvPoradca("md", "Ing. Miroslav Dzurovčin", "MD"),
  hvPoradca("pb", "Mgr. Peter Babjak",       "PB"),
  hvPoradca("kl", "Kvetka Luteranová",       "KL"),
  hvPoradca("mh", "Matúš Horský",            "MH"),
  hvPoradca("fi", "František Ivanov",         "FI"),
  hvPoradca("eg", "Mgr. Eva Gerthofferová",  "EG"),
...
function renderAll(){
  ...
  renderZTlacidla(v); renderSum(v); renderDalsieKroky(); renderServis(); renderKand();
}
...
let servis = null;          // null = klient sa nerozhodol, riadok sa do PDF nedostane
let zSeq = 1;
let poslZivot = null, poslDoch = null;

function zUloz(){
  try { sessionStorage.setItem("zhr2", JSON.stringify({z:zhrnutie, v:[...videne], k:krokyText, s:zSeq, srv:servis})); } catch(e){}
}
function zNacitaj(){
  try {
    ...
    if (d.srv === "plny" || d.srv === "jedno") servis = d.srv;
...
/* ---- rozsah spolupráce ---- */
const SERVIS = {
  plny: "Klient má záujem o kompletný servis a starostlivosť — pravidelné prehodnotenie zmlúv, " +
        "servis pri zmenách v živote a pomoc pri poistných udalostiach.",
  jedno: "Klient si volí jednorazové riešenie bez ďalšieho servisu a starostlivosti."
};
function renderServis(){
  document.querySelectorAll("#servisVolba .sw").forEach(b =>
    b.classList.toggle("on", b.dataset.servis === servis));
  $("sumServis").innerHTML = servis
    ? `<div class="zsek"><h3>Rozsah spolupráce</h3><p>${SERVIS[servis]}</p></div>` : "";
}
...
function renderSum(v){
  ...
  /* meno poradcu je v hlavičke vždy — aj pri značke so širokým logom,
     lebo pod jednou značkou pracuje viac poradcov */
  let h = `<div class="pdfhd">${markHTML(poradca,"mk")}
      <div class="pdfme"><b>${poradca.meno}</b><span>${poradca.rola}</span></div>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("servisVolba").addEventListener("click", e => {
    const b = e.target.closest("[data-servis]"); if (!b) return;
    servis = (servis === b.dataset.servis) ? null : b.dataset.servis;
    zUloz(); renderServis();
  });
  zNacitaj();
